...
function item_craft_overlay(craft_struct){
	//Get result item
	var item_craft = return_struct_from_item_index_by_item_id(craft_struct[$ "item_crafted"])
	//Setup item data
	...
	var over_hint_width = max(item_name.get_width()+50,string_width(craft_word_can_craft))
	...
	for(var i=0;i<array_length(craft_struct.required_items);i++){
	...
	draw_sprite_stretched(craft_button_spr,0,button_xx,button_yy,over_hint_width,40)
	draw_set_halign(fa_center)
	draw_set_valign(fa_middle)
	draw_text_shadow(button_xx+over_hint_width/2,button_yy+20,craft_word_can_craft,fnt_small,1,c_black,c_white,1)
}
