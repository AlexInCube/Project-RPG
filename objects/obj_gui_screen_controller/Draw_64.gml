...
if surface_exists(buttons_surf){
	surface_set_target(buttons_surf)
	draw_clear_alpha(c_white,0)
	//Draw name of current screen
	draw_set_halign(fa_center)
	draw_set_valign(fa_bottom)
	draw_text_shadow(GUIWIDTH/2,buttons_screen_start_y+5,global.screen_map[? current_screen][$ "screen_name"],fnt_large,2,c_black,c_white,1)
	...
	for(var i = 0;i<array_length(buttons_order);i++){
		...
		draw_sprite(global.screen_map[? buttons_order[i]][$ "screen_icon"],0,xx,yy)
