...
for(i = 0;  i < ds_grid_width(global.craft_map); i++)
	{
		if global.crafting_inventory[# 0,0] == global.craft_map[# i, 1] and global.crafting_inventory[# 1,0] == global.craft_map[# i, 3]
		{
			if global.crafting_inventory[# 0,1] == global.craft_map[# i, 2] and global.crafting_inventory[# 1,1] == global.craft_map[# i, 4]
			{
				...
				global.mouse_slot[# 0,0]=global.craft_map[# i, 0]
				global.mouse_slot[# 0,1]+=global.craft_map[# i, 5]
			
				}
			
				if global.craft_map[# i, 0]!=item.none
				{
				draw_sprite(global.item_index[# global.craft_map[# i, 0],item_stat.sprite_index],0,xx+16,yy+16)
				}
			}
		}
	}
