function scr_weapon_equip() {
	...
	if !instance_exists(obj_weapon_controller) and global.equipment[# 4, 0] != item.none{
		...
		with(inst){
			weapon_tick_script = global.item_index[# global.equipment[# 4, 0], item_stat.weapon_tick]
			weapon_draw_script = global.item_index[# global.equipment[# 4, 0], item_stat.weapon_draw]
			weapon_create_script = global.item_index[# global.equipment[# 4, 0], item_stat.weapon_create]
			weapon_alarm_script = global.item_index[# global.equipment[# 4, 0], item_stat.weapon_alarm]
